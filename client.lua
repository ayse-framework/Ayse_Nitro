...
DecorRegister("AYSE_NITRO_STATUS", 2)
DecorRegister("AYSE_NITRO_NOS", 1)
DecorRegister("AYSE_NITRO_PURGE", 1)

function hasNOS(veh)
    if not DecorExistOn(veh, "AYSE_NITRO_STATUS") then
        return false
    end
    return DecorGetBool(veh, "AYSE_NITRO_STATUS")
end

function setNOS(veh, status)
    DecorSetBool(veh, "AYSE_NITRO_STATUS", status)
end

function getValuesNOS(veh)
    return DecorGetFloat(veh, "AYSE_NITRO_NOS"), DecorGetFloat(veh, "AYSE_NITRO_PURGE")
end
...
exports("nos", function(data, slot)
    ...
    setNOS(veh, true)
    DecorSetFloat(veh, "AYSE_NITRO_NOS", 100.0)
    DecorSetFloat(veh, "AYSE_NITRO_PURGE", 0.0)
...
RegisterCommand("+purge", function()
    ...
    TriggerServerEvent("Ayse_Nitro:purge", true)
end, false)
...
RegisterCommand("-purge", function()
    ...
    TriggerServerEvent("Ayse_Nitro:purge", false)
end, false)
...
RegisterCommand("+nitro", function()
    ...
    TriggerServerEvent("Ayse_Nitro:flames", true)
end, false)
...
RegisterCommand("-nitro", function()
    ...
    activated = false
    TriggerServerEvent("Ayse_Nitro:flames", false)
    SetVehicleBoostActive(veh, activated)
    ...
    while true do
        ...
        if veh ~= 0 and hasNOS(veh) then
            ...
            if activated and noslvl > 0 then
                ...
                DecorSetFloat(veh, "AYSE_NITRO_NOS", lvl)
                SendNUIMessage({
                    type = "nosLevel",
                    nos = lvl
                })
                ...
                if purgelvl < 100 then
                    ...
                    DecorSetFloat(veh, "AYSE_NITRO_PURGE", lvl)
                    SendNUIMessage({
                        type = "purgeLevel",
                        purge = lvl
                    })
	...
	while true do
		...
		elseif isInVehicle then
			if not IsPedInAnyVehicle(ped, false) or IsPlayerDead(player) then
                ...
                if activated then
                    activated = false
                    TriggerServerEvent("Ayse_Nitro:flames", false, VehToNet(veh))
                    SetVehicleBoostActive(veh, activated)
                    SetVehicleCheatPowerIncrease(veh, 1.0)
                end
                if screen then
                ...
                if purging then
                    purging = false
                    TriggerServerEvent("Ayse_Nitro:purge", false, VehToNet(veh))
                end
                if display then
                    display = false
                    SendNUIMessage({
